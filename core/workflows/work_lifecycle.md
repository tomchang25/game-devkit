...
- The Goal and Summary provide a reviewable intent surface.
...
- The handoff has been revalidated against the current worktree.
...
| Resolve, specify, implement, and verify focused work | `commands/implement.md` | Non-destructive mutations inside the named target; conditional stops preserve user-authority boundaries |
...
`/implement` is the user-facing orchestration command for one focused target. Its invocation authorizes the target's non-destructive repository changes, and its command contract owns conditional stop conditions for unresolved user decisions, scope changes, external authority, destructive actions, and verification gaps. This lifecycle owns artifact readiness rather than conversational checkpoints. The implementation spec carries the durable handoff, and local code details are read just in time during implementation.
