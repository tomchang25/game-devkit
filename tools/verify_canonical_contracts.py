...
REQUIRED: dict[str, tuple[str, ...]] = {
    ...
    "core/agent_rules/change_design.md": (
        "## Direct Change",
        "update the API and every known consumer in the same landing",
        "Do not add speculative wrappers",
        "## Module And Dependency Boundaries",
        "reduce total complexity",
        "## Research Threshold",
        "Persistence changes also follow",
    ),
    ...
    "core/agent_rules/save_migrations.md": (
        "## When To Create A Migration",
        "A shipped or explicitly supported version",
    ...
    "core/workflows/commands/implement.md": (
        ...
        "authorizes the non-destructive repository mutations",
        "## Language And Artifact Lanes",
        "## Conditional Stop Conditions",
        "Stops are conditional, not scheduled",
        "## Stage 1: Resolve The Target",
        "## Stage 2: Build Or Revalidate The Handoff",
        "## Stage 3: Implement And Verify",
        "continue directly into implementation without requesting confirmation",
    ...
    "platforms/web-react/standards/project_structure_standard.md": (
        ...
        "a prefix such as `/debugger` remains an ordinary route",
        "One read-only catalog is the source for both hub listing and exact tool dispatch",
        "outside the production-reachable module graph",
        "Interactive debug tools issue canonical commands through the normal runtime gateway",
